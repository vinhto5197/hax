...
from packages.core.agent.harness import stream_completion_agentic
from packages.core.llm.client import DEFAULT_MODEL
from packages.core.schemas.chat import ChatRequest

router = APIRouter(prefix="/chat-agentic", tags=["chat"])

# Behaviour prompt for the agentic route. BEHAVIOUR only — NOT a tool inventory:
...
@router.post("")
async def chat_agentic(payload: ChatRequest) -> StreamingResponse:
    ...
    # Replay prior turns (de-amnesia), same as the other routes. Retrieval is NOT
    # applied here — the model calls search_documents as a tool when it wants it.
    messages = await load_history(conversation_id)
    # Optional per-request model override (UI dropdown); else the default.
    model = payload.model or DEFAULT_MODEL
    # Bind system + model into the harness; event_stream drives it and
    # serializes its {"content"}/{"status"} events to SSE.
...
"""
curl -N -X POST http://localhost:8000/api/chat-agentic \
